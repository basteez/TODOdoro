...
- [ ] Task 1: Configure React Flow zoom behaviour (AC: #1, #5)
  - [ ] Set `zoomOnScroll={true}` (default) on `<ReactFlow>`
  - [ ] Set `minZoom` and `maxZoom` to reasonable bounds (e.g., `0.1` to `4`)
  - [ ] Zoom centres on cursor position — React Flow default behaviour, verify it works
  - [ ] Test with multiple cards to confirm 60fps at various zoom levels
- [ ] Task 2: Configure React Flow pan behaviour (AC: #2, #5)
  - [ ] Set `panOnDrag={true}` for left-click drag on empty canvas areas (React Flow default)
  - [ ] Verify Space + drag pans (React Flow supports this via `panOnScroll` or keyboard modifier)
  - [ ] Verify middle-mouse-button drag pans — configure `panOnDrag` with button array: `panOnDrag={[1, 2]}` (middle + right) or use React Flow's `panActivationKeyCode="Space"`
  - [ ] Ensure card drag takes priority over canvas pan when clicking on a node
- [ ] Task 3: Configure tablet touch interactions (AC: #4)
  - [ ] Two-finger pinch zoom: React Flow handles this natively via browser touch events
  - [ ] Single-finger drag on card: moves card (React Flow default)
  - [ ] Single-finger drag on empty area: pans canvas
  - [ ] Test with Chrome DevTools touch emulation
  - [ ] Ensure minimum 44×44px touch targets on interactive elements
- [ ] Task 4: Verify no domain events from viewport changes (AC: #3)
  - [ ] Confirm: pan and zoom do NOT trigger any event store writes
  - [ ] Pan/zoom is purely visual — React Flow viewport state, not persisted
  - [ ] Only `TodoPositionedEvent` (from card drag, Story 2.5) writes to event store
